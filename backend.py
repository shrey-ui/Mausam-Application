...
def geocoder(location):
    geodata1 = requests.get(
        '''https://geocode.search.hereapi.com/v1/geocode?q={}&apiKey={}'''.format(strconvert(location), apikeyHERE))

    for j in geodata1.json()['items']:
        return (j['position'])


def main_data(curr_location, dest_location, precisin):
    ...
    if (precision >= 1 and precision <= 5):
        ...
        fdir = open(r'Directions.txt', 'w')

        direction_req = requests.get(
            'https://api.openrouteservice.org/v2/directions/driving-car?api_key=5b3ce3597851110001cf6248c7a32d7041a74717955795c64a764975&start={},{}&end={},{}'.format(
                curr_dest_coor['lng'], curr_dest_coor['lat'], dest_loc_coor['lng'], dest_loc_coor['lat']))
        ...
        webbrowser.open(
            'https://dev.virtualearth.net/REST/v1/Imagery/Map/Road/Routes?wp.0={};64;1&wp.1={};66;2&mapsize=3840,3840&key={}'.format(
                curr_loc, dest_loc, api_id_route))

        req = requests.get(
            'http://router.project-osrm.org/route/v1/driving/{},{};{},{}?alternatives=false&annotations=nodes'.format(
                curr_dest_coor['lng'], curr_dest_coor['lat'], dest_loc_coor['lng'], dest_loc_coor['lat']))
        ...
        for j in coorlist:
            ...
            reqweather = requests.get(
                'https://api.openweathermap.org/data/2.5/onecall?lat={}&lon={}&exclude=hourly,daily,minutely&appid={}'.format(
                    j[1], j[0], apikeyweather))
            ...
            if 'alerts' in reqweather.json():
                ...
                f1.write('ALERTS (courtesy {}): {}\n\n\n'.format(reqweather.json()['alerts'][0]['sender_name'],
                                                             reqweather.json()['alerts'][0]['description']))
